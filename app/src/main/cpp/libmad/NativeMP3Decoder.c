...
int NativeMP3Decoder_init(char *filepath, unsigned long start/*,unsigned long size*/) {
    ...
    __android_log_print(ANDROID_LOG_INFO, TAG, "start = %lu*******\n", start);
    ...
    do {
        if (mp3->stream.buffer == 0 || mp3->stream.error == MAD_ERROR_BUFLEN) {
            ...
            if (mp3->stream.next_frame != 0) {
                ...
                int readBytes = file_read(mp3->file, mp3->inputBuffer + leftOver,
                                          INPUT_BUFFER_SIZE - leftOver);
                if (readBytes == 0)
                    return 0;
...
int NativeMP3Decoder_readSamples(short *target, int size) {
    ...
    while (idx != size) {
        if (mp3->leftSamples > 0) {
            for (; idx < size &&
                   mp3->offset < mp3->synth.pcm.length; mp3->leftSamples--, mp3->offset++) {
